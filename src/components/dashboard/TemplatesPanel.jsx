...
export default function TemplatesPanel({ customTemplates, onCreate }) {
  ...
  return <section aria-label="Template library"><div className="template-filters" aria-label="Template categories">{['All', 'Starter', ...THEME_CATEGORIES, 'Custom'].map((item) => <button key={item} aria-pressed={category === item} onClick={() => setCategory(item)}>{item}</button>)}</div>
    {error && <p role="alert" className="ui-error">{error}</p>}
    <div className="deck-grid">{templates.filter((item) => category === 'All' || item.category === category).map((item) => <button key={item.id} disabled={pending !== null} onClick={() => create(item)} className="template-tile"><div className="template-preview"><SlideView slide={parseMarpPresentation(item.markdown).slides[0]} theme={item.theme} showSlideNumber={false} /></div><div className="template-caption"><div><h3>{item.title}</h3><p>{pending === item.id ? 'Creating your deck…' : item.category}</p></div><ArrowUpRight size={18} /></div></button>)}</div>
